...
func (j *Jwt) GenerateToken(userID string, duration time.Duration) (string, error) {
	now := time.Now()
	claims := CustomClaims{
	...
	if err != nil {
		if result, ok := err.(jwt.ValidationError); ok {
			...
			case result.Errors&jwt.ValidationErrorMalformed != 0:
				return EmptyString, errors.New(TokenMalformed)
			case result.Errors&jwt.ValidationErrorExpired != 0:
				return EmptyString, errors.New(TokenExpired)
			case result.Errors&jwt.ValidationErrorNotValidYet != 0:
				return EmptyString, errors.New(TokenNotValidYet)
			default:
				return EmptyString, errors.New(TokenInvalid)
			}
		}
...
func (j *Jwt) RefreshToken(tokenStr string) (string, error) {
	...
	if claims, ok := token.Claims.(*CustomClaims); ok && token.Valid {
		...
		return j.GenerateToken(claims.UserID, duration)
	}
